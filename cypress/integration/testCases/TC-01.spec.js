...
describe("TC-01: Lowest price reservation", () => {
    ...
    it("Should reserve the third hotel with the lower price", () => {
        ...
        booking.getFirstNameField().type(testData.firstName)
        booking.getLastNameField().type(testData.lastName)
        booking.getEmailField().type(testData.email)
        booking.getEmailConfirmationField().type(testData.email, { force: true })
        booking.getFullNameField().type(testData.fullName)
        ...
        cy.get('.field_name_full_name > .book-form-field-value').invoke('text').then((text) => {
            ...
            expect(text.replaceAll('\n', ' ')).to.contain(testData.fullName)
        })
        cy.get('.personal_details_reassurance_email_text').invoke('text').then((text) => {
            //cy.wrap(text).should('contain.text', 'Pepe Perez')
            expect(text.replaceAll('\n', ' ')).to.contain(testData.email)
        })
